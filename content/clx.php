...
include "includes/upperheader.php";
   echo "<title>CLX boxes and unboxed ('E' accessions)</title>";
   include "includes/lowerheader.php";
	 echo "<h1><a href='".$_SERVER['PHP_SELF']."'>CLX boxes and unboxed ('E' accessions)</a></h1>";
	 
	 if (!isset($view)) { 
echo "<form method='GET' action='".$PHP_SELF."'>";
echo "<input type='hidden' name='view' value='interim' />";	
echo "Select year: <select name='year'>";	 
$sql_str="SELECT distinct year FROM cms_clx2 WHERE year>0 ORDER BY year ASC";
...
$sql_str="SELECT distinct number FROM cms_clx2 WHERE year LIKE '$year' AND number>0 ORDER BY number ASC";
$boxsearch = mysqli_query($id_link, $sql_str) or die("Search Failed!");
while ($results = mysqli_fetch_array($boxsearch)):	
...
include "msaccession.php";
	 ...
	 $sql_str="SELECT * FROM cms_clx2 ORDER BY year ASC, number ASC";
$boxsearch = mysqli_query($id_link, $sql_str) or die("Search Failed!");
